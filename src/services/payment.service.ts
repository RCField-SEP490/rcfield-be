...
export async function createCheckoutUrl(
  ...
  if (!existingTx) {
    const tx = txRepo.create({
      ...
      rawRequest: { bookingId, totalCharged, ipAddr, gateway: gateway.name },
    });
...
export async function mockConfirmPayment(
  ...
  const contestAdj = await resolveContestPricingAdjustments(booking);
  const slotFee = contestAdj.waiveSlotFee ? 0 : rawSlotFee;
  const adjustedDepositByVehicleId = new Map(
    bookingVehicles.map((v) => [
      v.id,
      Math.round(Number(v.securityDepositSnapshot) * contestAdj.depositMultiplier),
  ...
  const grossMockTotal = slotFee + rentalFeeTotal + adjustedDepositTotal + fnbTotal;
